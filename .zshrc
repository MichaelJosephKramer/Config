...
# fzf — keybindings and completion come straight from the binary (`fzf --zsh`,
# supported since 0.48). This replaced sourcing ~/.fzf.zsh, a file generated by
# the old vim-plug fzf install that also put a second, shadowed fzf on PATH.
command -v fzf >/dev/null && source <(fzf --zsh)
export FZF_DEFAULT_COMMAND='fd --type f --hidden --follow --exclude .git'
# Ctrl-T otherwise falls back to fzf's built-in walker, which skips hidden files
# entirely — in a dotfiles repo that hides most of the tree.
